...
function setupContainer(container: Element) {
  ...
  try {
    const parsed = JSON.parse(dataEl.textContent ?? "[]") as GlossaryHeadword[]
    entries = indexGlossaryEntries(parsed)
  } catch {
    return
  }

  let current: GlossaryHeadword[] = []
  let activeIndex = -1
  ...
  const render = (matches: GlossaryMatch[], queryLength: number) => {
    results.replaceChildren()
    current = matches.map((match) => match.entry)
    activeIndex = -1

    for (const { entry, headwordIndex, glossIndex, aliasIndex, aliasMatchIndex } of matches) {
      ...
      if (headwordIndex < 0 && aliasIndex >= 0) {
        headword.append(document.createTextNode(" · "))
        headword.append(highlighted(entry.aliases![aliasIndex], aliasMatchIndex, queryLength))
      }
      a.append(headword)
  ...
  const onInput = () => {
    ...
    const shown = searchGlossary(entries, query)
    render(shown, fold(query).length)
