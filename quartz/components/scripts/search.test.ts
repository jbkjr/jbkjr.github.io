...
const encoder = (str: string): string[] => {
  ...
  const normalizeToken = (token: string) => token.normalize("NFD").replace(/\p{Diacritic}/gu, "")
  ...
  for (const char of lower) {
    ...
    if (isCJK) {
      if (bufferStart !== -1) {
        tokens.push(normalizeToken(lower.slice(bufferStart, bufferEnd)))
        bufferStart = -1
      }
      tokens.push(char)
  ...
  if (bufferStart !== -1) {
    tokens.push(normalizeToken(lower.slice(bufferStart)))
  }
...
describe("search encoder", () => {
  describe("English text", () => {
    ...
    test("should normalize diacritics without changing token boundaries", () => {
      const result = encoder("Taṇhā saṅkhāra")
      assert.deepStrictEqual(result, ["tanha", "sankhara"])
    })
  })
